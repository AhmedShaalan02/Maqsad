...
// Tafseer mode deck: verse cards + quiz every 5
function buildDeck(verses) {
  ...
  for (let i = 0; i < verses.length; i += 5) {
    ...
    const qv = group[Math.floor(Math.random() * group.length)]
    const wrongs = shuffle(verses.filter(v => v.id !== qv.id))
    deck.push({
      type: 'quiz',
      ...
      options: shuffle([
        { text: truncate(cleanText(qv.translations[0].text), 130), correct: true },
        ...wrongs.slice(0, 3).map(v => ({
          text: truncate(cleanText(v.translations[0].text), 130),
          correct: false,
        })),
      ]),
    })
...
const SW = new Set(['the','a','an','in','of','to','and','or','but','is','are','was','were','be','been','have','has','had','do','does','did','will','would','could','should','may','might','with','by','at','from','for','on','as','it','its','this','that','he','she','they','we','you','his','her','their','our','your','not','then','than','so','if','all','who','whom','when','where','which','what','how','unto','upon','whom','thee','thy','thou'])
...
function TilawahView({ verses, surahName, onComplete }) {
  ...
  return (
    <>
      ...
      <div ref={scrollRef} className="flex-1 overflow-y-auto" onScroll={onScroll}>
        ...
        {verses.map(v => <AyahBlock key={v.id} verse={v} />)}
...
function HifzCard({ verse, onResult, onNext, animDir, isLast }) {
...
function TafseerCard({ item, surahName, animDir }) {
  const { verse } = item
  ...
  return (
    <div className="mx-4 rounded-3xl overflow-hidden" style={{ backgroundColor: '#FFFCF7', boxShadow: '0 12px 40px rgba(28,20,16,0.13), 0 2px 8px rgba(28,20,16,0.06)', animation: `${animDir === 'right' ? 'slideInRight' : 'slideInLeft'} 0.28s cubic-bezier(0.25,0.46,0.45,0.94) both` }}>
      {/* Arabic */}
      ...
      {tafseer ? (
        <>
          ...
          <div className="px-6 pt-3 pb-5">
            <p className="text-xs font-bold uppercase tracking-wider mb-2" style={{ color: '#C4973A' }}>Tafseer · Ibn Kathir</p>
            <p style={{ fontSize: '12px', color: 'rgba(28,20,16,0.62)', lineHeight: '1.75' }}>{truncate(tafseer, 380)}</p>
          </div>
        </>
      ) : (
        <div className="px-6 pb-4">
      ...
      <div className="flex items-center justify-between px-6 py-3" style={{ borderTop: '1px solid rgba(196,151,58,0.15)' }}>
        ...
        <div className="flex items-center gap-3">
          <a href={`https://quran.com/${sNum}/${vNum}`} target="_blank" rel="noreferrer" style={{ color: 'rgba(28,20,16,0.32)' }} onClick={e => e.stopPropagation()}><ExtLink /></a>
          <button style={{ color: 'rgba(28,20,16,0.32)' }}><FlagIco /></button>
        </div>
      </div>
    </div>
  )
    ...
    <div className="mx-4 rounded-3xl overflow-hidden" style={{ backgroundColor: '#FFFCF7', boxShadow: '0 12px 40px rgba(28,20,16,0.13)', animation: `${animDir === 'right' ? 'slideInRight' : 'slideInLeft'} 0.28s cubic-bezier(0.25,0.46,0.45,0.94) both` }}>
      ...
      <div className="px-4 pb-5 flex flex-col gap-2">
        {item.options.map((opt, i) => {
          ...
          return (
            <button key={i} onClick={() => !answered && onAnswer(i)} className="w-full text-left px-4 py-3 rounded-xl text-sm leading-snug transition-all" style={{ backgroundColor: bg, border: `1px solid ${border}`, color, cursor: answered ? 'default' : 'pointer' }}>
              <span className="inline-block w-5 text-center font-bold mr-2 text-xs" style={{ opacity: 0.45 }}>{String.fromCharCode(65 + i)}</span>
              {opt.text}
            </button>
          )
        })}
...
export default function QuranSessionPage() {
  ...
  const { surah, ayaatCount, mode } = state || {}
  ...
  useEffect(() => {
    ...
    loader().then(async mod => {
      ...
      if (mode === 'Tafseer') {
        ...
        // Fetch Ibn Kathir tafseer from CDN (CORS-friendly jsdelivr)
        const tafseerResults = await Promise.all(
          sliced.map(v => {
            const [sn, vn] = v.verse_key.split(':')
            return fetch(`https://cdn.jsdelivr.net/gh/spa5k/tafsir_api@main/tafsir/en-tafisr-ibn-kathir/${sn}/${vn}.json`)
              .then(r => r.ok ? r.json() : null)
              .catch(() => null)
          })
        )
        const enriched = sliced.map((v, i) => ({ ...v, tafseer: tafseerResults[i]?.text || null }))
        setVerses(enriched)
        setDeck(buildDeck(enriched))
      } else if (mode === 'Hifz') {
        setVerses(sliced)
        setDeck(sliced.map(v => ({ type: 'hifz', verse: v })))
      } else {
        // Tilawah — just verses, no deck needed
        setVerses(sliced)
      }
  ...
  return (
    <div className="flex flex-col" style={{ height: '100dvh', backgroundColor: '#FAF7F2' }}>
      ...
      {!loading && !error && mode === 'Tilawah' && !showResults && (
        <TilawahView verses={verses} surahName={surah.name_simple} onComplete={() => setShowResults(true)} />
      )}
        ...
        <div className="flex-1 flex flex-col overflow-hidden" onTouchStart={onTouchStart} onTouchEnd={onTouchEnd}>
          ...
          <div className="flex-1 overflow-y-auto py-4 select-none">
            {currentCard.type === 'hifz' && (
              <HifzCard key={currentIndex} verse={currentCard.verse} onResult={handleHifzResult} onNext={goNext} animDir={enterDir} isLast={isLast} />
            )}
            {currentCard.type === 'verse' && (
              <TafseerCard key={currentIndex} item={currentCard} surahName={surah.name_simple} animDir={enterDir} />
            )}
            {currentCard.type === 'quiz' && (
              <QuizCard key={currentIndex} item={currentCard} quizState={quizState} onAnswer={handleQuizAnswer} animDir={enterDir} />
            )}
          </div>
          ...
          {mode === 'Tafseer' && (
            <div className="flex-shrink-0 flex items-center justify-between px-5 pb-6 pt-2" style={{ borderTop: '1px solid rgba(28,20,16,0.05)' }}>
              <button onClick={goPrev} disabled={currentIndex === 0} className="px-4 py-2 rounded-full text-sm font-medium"
